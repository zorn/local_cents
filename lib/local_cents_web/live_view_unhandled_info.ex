defmodule LocalCentsWeb.LiveViewUnhandledInfo do
  ...
  receives — an unmatched one raises `FunctionClauseError` and crashes the view.
  Without this hook, each view that subscribes to PubSub must hand-write a no-op
  clause for every signal it does not care about, and that boilerplate grows
  with every new signal type (see [ADR
  0019](0019-liveview-unhandled-info-fallback.html)).
  ...
  first; a catch-all defined early would shadow them.
  """
  ...
  defmacro __before_compile__(_env) do
    quote do
      ...
      def handle_info(message, socket) do
        Logger.warning("#{inspect(__MODULE__)} ignored an unhandled message: #{inspect(message)}")

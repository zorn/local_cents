...
1. The register conflicts (§1) — it is a scalar map key written on both sides.
2. Automerge deterministically picks one winner by operation id (§1) and retains the other as a loser in the multi-value register (§2).
3. `get_all` in the core crate could enumerate both candidates (§3), but our `decode`/`hydrate` path reads only the winner and discards the loser (§4).

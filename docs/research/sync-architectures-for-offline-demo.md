...
## Prior art: nteract 2.0 — the split-peer anchor for "do it properly"

[nteract](https://github.com/nteract/nteract) began in 2016 as an Electron notebook app whose document model was React + Redux over an immutable, operation-based structure ([`@nteract/commutable`](https://github.com/nteract/commutable)). **nteract 2.0** discards that: it is a ground-up rewrite in **Rust + TypeScript on Tauri** — our exact shell — in which **every notebook is an Automerge CRDT document** ([nteract 2.0 blog](https://nteract.io/blog/nteract-2.0)). Cite **2.0** specifically. The 1.x lineage has no Automerge in it, so a reader who opens the old codebase or the `commutable` package finds a different architecture.

It is the anchor because it has already shipped the shape this note weighs. Its frontend and its runtime daemon are **each their own Automerge peer**, both compiled from the same Rust `automerge` crate — the daemon natively, the frontend to WASM (`runtimed-wasm`: "WASM bindings … compiled from the same automerge crate as the daemon", [Cargo.toml](https://raw.githubusercontent.com/nteract/nteract/main/crates/runtimed-wasm/Cargo.toml)). They converge by exchanging **Automerge sync messages over a length-prefixed socket protocol** (Unix domain sockets / named pipes), **not** through Tauri IPC commands into a single Rust-held document ([`notebook-sync/Cargo.toml`](https://raw.githubusercontent.com/nteract/nteract/main/crates/notebook-sync/Cargo.toml) depends on `automerge`, not `automerge-repo`). Crash isolation is the point they call out: "If the runtime goes down, it doesn't take your document down with it — you still have the copy you're editing" ([blog](https://nteract.io/blog/nteract-2.0)). That split-peer model is what Options A and B below reach for. nteract 2.0 also exposes an MCP server and CLI so an agent can join a document as a real-time Automerge collaborator ([blog](https://nteract.io/blog/nteract-2.0)) — orthogonal to sync, but the same peer mechanism.
...
1. **What must the demo literally show?** "Two app instances reconcile" (Option A satisfies) vs. "a browser tab, with its own local store, works while offline" (only Option B satisfies). The map's intent here decides between A and B and is the pivotal question.
   ...
   **Leaning — toward A, given the long-term hosted and self-hosted plan.** This note lays out options and does not choose; the decision belongs to the grilling ticket. But the product direction pushes hard on this fork, so record the argument here rather than re-derive it downstream. A hosted LocalCents, a self-hosted instance, and the Mac app's embedded BEAM are all **BEAM peers** that reconcile server-to-server through the existing `ExAutomerge` path — which is exactly the "second instance" Option A treats as the peer. So A's transport is the first leg of the real product, not throwaway demo scaffolding. In that world the browser staying a thin LiveView client is **correct, not a compromise**: a hosted or self-hosted browser talks to a real server by definition, and the offline-capable client is the Tauri app, which always has its local BEAM. Option B's serverless in-tab peer earns its high cost only if "a browser tab that keeps working with no server behind it at all" becomes an explicit **product** goal — pure-web users being online-only is a coherent local-first stance otherwise. Option C stays useful as a cheap stepping-stone to get the divergence-then-merge **UX** (#211/#212) in front of people while A's transport is built. This leaning flips only if the map wants the browser itself to be a first-class offline client with no server behind it.

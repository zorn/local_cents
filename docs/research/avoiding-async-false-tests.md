...
>
> **Two claims here have since been corrected** by [`test-isolation-patterns-for-global-state.md`](test-isolation-patterns-for-global-state.md), the companion note covering the LiveView feature tests. First, `$callers` is written *only* by `Task` — the sentence in 2d below that credits `GenServer`/`proc_lib` children with it is wrong, and is annotated inline. Second, and more consequentially, `$callers` **does** reach a LiveView started by `Phoenix.LiveViewTest`, so Option B is viable for the feature tests in a way this note concluded it was not. Read the companion note before acting on 2d or Option B.
>
> **Option B has since been implemented**, in [issue #147](https://github.com/zorn/local_cents/issues/147). `LocalCents.BooksDirHelper` no longer mutates the application env — it defines a directory for the test's process tree through `LocalCents.ProcessConfig`. Two things below therefore describe a state that no longer exists: the helper's moduledoc as quoted here, and the module list in "The problem, precisely". Every test module in the suite now runs `async: true`. The standard lives in the Testing section of `CODING_STANDARDS.md` and the seam in `LocalCents.ProcessConfig`; this note remains the evidence behind the injection half of the argument and is not maintained against the code.
...
> **Correction.** The claim that `GenServer`/`proc_lib` children get `$callers` is false. In the Elixir standard library, `$callers` is written in exactly one place — Task.Supervised (`task/supervised.ex:88`) — and the key appears only in `task.ex`, `task/supervised.ex`, and `task/supervisor.ex`. OTP's `proc_lib` sets `$ancestors`, a different key with different semantics, and mentions `callers` nowhere. A plain `GenServer.start_link/3` propagates neither. The conclusion this sentence supports still holds for `BookServer` — a `DynamicSupervisor` child is not reachable via `$callers` — but it holds for a broader reason than stated. See [`test-isolation-patterns-for-global-state.md`](test-isolation-patterns-for-global-state.md) for the probe table across five start paths, and for the finding that a `Phoenix.LiveViewTest`-driven LiveView *is* reachable.

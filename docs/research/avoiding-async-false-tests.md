...
>
> **Option B has since been implemented**, in [issue #147](https://github.com/zorn/local_cents/issues/147). `LocalCents.BooksDirHelper` no longer mutates the application env — it claims a directory for the test's process tree through `LocalCents.ProcessConfig` — so the quotations below of its moduledoc, and the module list in "The problem, precisely", describe a state that no longer exists. Every test module in the suite now runs `async: true`. The standing guide is [Async testing](async-testing.html); this note remains the evidence behind the injection half of the argument and is not maintained against the code.
